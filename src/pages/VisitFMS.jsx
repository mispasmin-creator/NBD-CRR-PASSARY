        ...
        <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100">
            <div className="py-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                    <div className="bg-card rounded-lg shadow-md p-4"><div className="text-sm text-muted-foreground">Scheduled</div><div className="text-2xl font-bold text-blue-600">{visits.filter(v => v.status === "Scheduled").length}</div></div>
                    <div className="bg-card rounded-lg shadow-md p-4"><div className="text-sm text-muted-foreground">Completed</div><div className="text-2xl font-bold text-green-600">{visits.filter(v => v.status === "Completed").length}</div></div>
                    <div className="bg-card rounded-lg shadow-md p-4"><div className="text-sm text-muted-foreground">Cancelled</div><div className="text-2xl font-bold text-red-600">{visits.filter(v => v.status === "Cancelled").length}</div></div>
                </div>
                ...
                <div className="bg-card rounded-lg shadow-md overflow-hidden">
                    <table className="w-full">
                        <thead className="bg-muted border-b">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Sales Rep</th>
                                <th className="px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Date & Time</th>
                        ...
                        <tbody className="divide-y">
                            {filteredVisits.map((v) => (
                                <tr key={v.id} className="hover:bg-muted">
                                    ...
                                    <td className="px-6 py-4 text-muted-foreground">{v.salesRep}</td>
                    ...
                    {filteredVisits.length === 0 && <div className="text-center py-12 text-muted-foreground">No visits found.</div>}
                </div>
            </div>
        </div>
    )
