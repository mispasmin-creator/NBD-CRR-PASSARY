        ...
        <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100">
            <div className="py-2">
                <div className="flex flex-col md:flex-row justify-end items-start md:items-center mb-8">
                    <div className="flex gap-2 mt-4 md:mt-0">
                        {["week", "month", "quarter", "year"].map(range => (
                            <button key={range} onClick={() => setDateRange(range)} className={`px-4 py-2 rounded-md font-medium capitalize ${dateRange === range ? "bg-sky-600 text-white" : "bg-card text-muted-foreground hover:bg-muted/50"}`}>{range}</button>
                        ))}
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                        ...
                        <div className="flex items-end justify-between h-48 gap-2">
                            {monthlyTrend.map((m, i) => (
                                <div key={i} className="flex flex-col items-center flex-1">
                                    ...
                                    <div className="text-xs text-muted-foreground mt-2">{m.month}</div>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    {/* Sales by Region */}
                    <div className="bg-card rounded-lg shadow-md p-6">
                        <h2 className="text-lg font-semibold text-foreground mb-4">Sales by Region</h2>
                    ...
                    <table className="w-full">
                        <thead className="bg-muted">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Total Sales</th>
                                <th className="px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Deals Closed</th>
                                <th className="px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Conversion Rate</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y">
                            {salesByRep.map((rep, i) => (
                                <tr key={i} className="hover:bg-muted">
                                    ...
                                    <td className="px-6 py-4 text-muted-foreground">₹{(rep.sales / 100000).toFixed(1)}L</td>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-muted-foreground">{rep.conversion}%</span>
                                        </div>
                                    </td>
                                </tr>
                            ))}
