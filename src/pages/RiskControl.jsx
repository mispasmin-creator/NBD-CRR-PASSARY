        ...
        <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100">
            <div className="py-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-8">
                    <div className="bg-card rounded-lg shadow-md p-4"><div className="text-sm text-muted-foreground">Total Risks</div><div className="text-2xl font-bold text-foreground">{riskMatrix.length}</div></div>
                    <div className="bg-card rounded-lg shadow-md p-4"><div className="text-sm text-muted-foreground">Active Risks</div><div className="text-2xl font-bold text-red-600">{riskMatrix.filter(r => r.status === "Active").length}</div></div>
                    <div className="bg-card rounded-lg shadow-md p-4"><div className="text-sm text-muted-foreground">Monitored</div><div className="text-2xl font-bold text-yellow-600">{riskMatrix.filter(r => r.status === "Monitored").length}</div></div>
                    <div className="bg-card rounded-lg shadow-md p-4"><div className="text-sm text-muted-foreground">Controlled</div><div className="text-2xl font-bold text-green-600">{riskMatrix.filter(r => r.status === "Controlled").length}</div></div>
                </div>

                {/* Risk Matrix */}
                <div className="bg-card rounded-lg shadow-md mb-6">
                    <div className="p-6 border-b flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <h2 className="text-lg font-semibold text-foreground">Risk Assessment Matrix</h2>
                        <div className="flex gap-2 flex-wrap">
                            {categories.map(c => (
                                <button key={c} onClick={() => setSelectedCategory(c)} className={`px-3 py-1 rounded-md text-sm capitalize ${selectedCategory === c ? "bg-sky-600 text-white" : "bg-muted/50 text-muted-foreground hover:bg-slate-200"}`}>{c}</button>
                            ))}
                        </div>
                    </div>
                    <table className="w-full">
                        <thead className="bg-muted">
                            <tr>
                                <th className="px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Risk</th>
                                <th className="px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Category</th>
                                <th className="px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Likelihood</th>
                                <th className="px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Impact</th>
                                <th className="px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Status</th>
                                <th className="px-6 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Mitigation</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y">
                            {filteredRisks.map(r => (
                                <tr key={r.id} className="hover:bg-muted">
                                    <td className="px-6 py-4 font-medium text-foreground">{r.risk}</td>
                                    ...
                                    <td className="px-6 py-4 text-muted-foreground text-sm">{r.mitigation}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                            ...
                            {complianceStatus.map((c, i) => (
                                <div key={i} className="flex justify-between items-center p-3 bg-muted rounded-lg">
                                    <div>
                                        ...
                                        <div className="text-sm text-muted-foreground">Last reviewed: {c.lastReview}</div>
                                    </div>
                        ...
                        <div className="space-y-3">
                            {auditLogs.map((l, i) => (
                                <div key={i} className="border-b border-slate-100 pb-3 last:border-0">
                                    ...
                                    <div className="text-sm text-muted-foreground">{l.user} - {l.details}</div>
                                </div>
                            ))}
                        </div>
                    </div>
