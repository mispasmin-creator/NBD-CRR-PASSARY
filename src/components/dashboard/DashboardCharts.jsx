...
const fallbackConversionData = [
  ...
  { name: "Enquiries", value: 82, color: "#4F46E5" },  // Bright Violet
...
const fallbackSourceData = [
  ...
  { name: "Justdial", value: 28, color: "#4F46E5" },      // Bright cyan
    ...
    <div className="flex flex-col h-full mt-2">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4 border-b border-slate-100 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xl md:text-2xl font-extrabold text-foreground tracking-tight">Sales Analytics</h3>
            <p className="text-xs text-muted-foreground font-medium tracking-wide">( Lead → Order )</p>
          </div>
        </div>
      ...
      {isLoading ? (
        <div className="h-[400px] flex flex-col items-center justify-center space-y-4">
          ...
          <p className="text-muted-foreground font-medium animate-pulse">Analyzing sales data...</p>
        </div>
      ) : error ? (
        ...
        <div className="h-[400px] w-full bg-card rounded-3xl">
          {activeTab === "overview" && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 h-full">
              <div className="h-full w-full flex items-center justify-center relative">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <span className="text-3xl font-black text-foreground">{conversionData[0]?.value || 0}</span>
                  <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Total Leads</span>
                </div>
              </div>

              <div className="flex flex-col justify-center overflow-y-auto max-h-[400px] px-2 md:px-6">
                <h4 className="text-sm font-bold uppercase tracking-widest text-muted-foreground mb-6 drop-shadow-sm">Conversion Funnel Drop-off</h4>
                <div className="space-y-6">
                  {conversionData.map((item, index) => (
                    <div key={index} className="space-y-2 group">
                      <div className="flex justify-between items-end">
                        <span className="text-sm font-extrabold text-muted-foreground tracking-tight group-hover:text-indigo-600 transition-colors">{item.name}</span>
                        <div className="flex flex-col items-end">
                          <span className="text-xl font-black text-foreground">{item.value}</span>
                        </div>
                      </div>
                      <div className="w-full bg-muted/50 rounded-full h-3.5 shadow-inner overflow-hidden relative">
                        <div
