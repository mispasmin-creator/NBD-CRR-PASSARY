...
function MetricCard({ title, value, change, trend, icon, color, dateRange }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-slate-200 overflow-hidden group">
      ...
      <div className="p-6">
        <div className="flex justify-between items-start">
          ...
          <div className={`p-3.5 rounded-2xl bg-gradient-to-br ${color} text-white shadow-md transform group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300`}>{icon}</div>
        </div>
        <div className="flex items-center mt-6 bg-slate-50 px-3 py-2 rounded-xl border border-slate-100 inline-flex w-full">
          {trend === "up" ? (
            <TrendingUpIcon className="h-4 w-4 text-emerald-500 mr-2" />
          ) : (
            <AlertCircleIcon className="h-4 w-4 text-rose-500 mr-2" />
          )}
          <span className={trend === "up" ? "text-emerald-700 font-semibold text-[13px]" : "text-rose-600 font-semibold text-[13px]"}>
