...
export const mockApi = {
    login: async (username, password) => {
        try {
            ...
            if (response.data && response.data.success) {
                ...
                const usernameIdx = normalizedHeaders.indexOf("username") !== -1
                    ? normalizedHeaders.indexOf("username")
                    : normalizedHeaders.indexOf("usernae");
                ...
                const lastLoginIdx = normalizedHeaders.indexOf("last_login");
                ...
                for (let i = 1; i < sheetData.length; i++) {
                    ...
                    if (rowUsername === username.trim() && rowPassword === password) {
                        const pageAccess = pageAccessIdx !== -1 ? (row[pageAccessIdx] ? row[pageAccessIdx].toString().trim() : "") : "";
                        ...
                        if (lastLoginIdx !== -1) {
                            ...
                            try {
                                ...
                                payload.append("rowIndex", String(i + 1));
                                payload.append("rowData", JSON.stringify(updatedRow));
                                await axios.post(scriptUrl, payload);
                            } catch (lastLoginError) {
                                console.error("Failed to update last_login:", lastLoginError);
                            }
                        }
                        ...
                        return {
                            ...
                            user: {
                                ...
                                userType: role === "admin" || pageAccess.toLowerCase() === "all" ? "admin" : "user",
    ...
    fetchDashboardAppCharts: async (currentUser, isAdminFunc) => {
        ...
        const sourceData = Object.keys(sourceCounter).map((name, index) => ({
            ...
            color: ["#4F46E5", "#0ea5e9", "#3b82f6", "#6366f1", "#8b5cf6"][index % 5]
        }));
